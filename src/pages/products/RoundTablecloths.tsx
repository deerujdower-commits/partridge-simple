...
const RoundTablecloths = () => {
  ...
  const selectedSize = sizes.includes(size || '') ? size : sizes[1]; // Default to 108"

  // Update URL when color changes
  const handleColorSelect = (colorSlug: string) => {
    navigate(`/collection/round-tablecloths/${colorSlug}${selectedSize ? `/${selectedSize}` : ''}`);
  };

  // Update URL when size changes
  const handleSizeSelect = (newSize: string) => {
    navigate(`/collection/round-tablecloths/${selectedProduct.slug}/${newSize}`);
  };
...
export default RoundTablecloths;